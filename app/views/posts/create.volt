...
<div class="container-fluid">
  <form action="" method="post" id="contentForm" class="form-horizontal" autocomplete="off">
    
    {% if post %}
      {% set id = post.getLast().id + 1 %}
      {{ hidden_field("new_post", "value" : "1") }}
    {% else %}
      {% set id = p.id %}
    {% endif %}
    ...
    {{ hidden_field("created_at", "size" : 30, "class" : "form-control tanggal", "id" : "fieldCreatedAt", "value" : date( 'Y-m-d H:i:s',time())) }}
    ...
    <div class="row">
      <div class="col-md-8">
        ...
        <div class="form-group">
          <textarea id="summernote" name="post_content">{{ p.content }}</textarea>
        </div>
        ...
        <div class="list-group">
          ...
          <div href="#" class="list-group-item">
            ...
            <p class="list-group-item-text">
              ...
              {% for c in categories %}
                <option value="{{ c.id }}" {% if c.id == p.category %}selected{% endif %}>{{ c.name }}</option>
              {% endfor %}
              </select>
            </p>
          </div>
          ...
          <div href="#" class="list-group-item">
            <h4 class="list-group-item-heading">Publish Date</h4>
            <p class="list-group-item-text">
              {% if post %}
                {{ text_field("published_at", "size" : 30, "class" : "form-control datetime", "id" : "fieldPublishedAt", "value" : date( 'Y-m-d H:i:s',time())) }}
              {% else %}
